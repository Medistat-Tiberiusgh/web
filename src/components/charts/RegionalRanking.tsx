...
export default function RegionalRanking({
  ...
  const natAvg =
    natStat?.per1000 ??
    (ranked.length > 0
      ? ranked.reduce((s, r) => s + r.per1000, 0) / ranked.length
      : null)
          ...
          return (
            ...
            >
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span
                  className={`text-xs flex-1 truncate ${isHome || isSelected ? 'font-semibold text-gray-900' : 'text-gray-700'}`}
                >
                  {region.regionName}
                </span>
                {isHome && (
                ...
                <span
                  className={`text-xs font-semibold tabular-nums ${isHome ? 'text-teal-600' : 'text-gray-500'}`}
                >
