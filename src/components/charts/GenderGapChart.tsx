...
export default function GenderGapChart({
  ...
  const userIsMale: boolean | null =
    user?.genderId != null ? user.genderId === 1 : null
    ...
    <div className="relative pt-1">
      ...
      >
        {(!filterGender || isMaleLike(filterGender)) && (
          <text
            x={effectiveCenterX - CENTER_W / 2 - 4}
            y={PAD.top - 4}
            textAnchor="end"
            fontSize={FONT_TICK}
        ...
        {years.map((year, i) => {
          ...
          return (
            <g key={year}>
              ...
              {/* Bars */}
              <rect
                x={effectiveCenterX - CENTER_W / 2 - regMenLen}
                y={y}
                width={regMenLen}
                height={dynBarH}
                rx={2}
              ...
              <rect
                ...
                onMouseEnter={(e) =>
                  setTooltip({
                    ...
                    y: e.clientY,
                    year,
                    regMen: hasRegional ? primary.men : null,
                    regWomen: hasRegional ? primary.women : null,
                    natMen: nat.men,
                    natWomen: nat.women
                  })
                }
        ...
        <line
          x1={effectiveCenterX - CENTER_W / 2}
          y1={PAD.top - 4}
          x2={effectiveCenterX - CENTER_W / 2}
          y2={H - PAD.bottom}
          stroke={COLOR_AXIS}
          strokeWidth={1}
        />
            ...
            >
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <span className="flex items-center gap-1 text-gray-500 whitespace-nowrap">
                          <span
                            className="w-2 h-2 rounded-full shrink-0"
                            style={{
                              background: hasRegional
                                ? COLOR_MALE_NAT
                                : menColor
                            }}
                          />
                          {maleLabel}
                          {!hasRegional && userIsMale === true ? ' · you' : ''}
                        </span>
                        <span className="font-semibold text-gray-700">
                          {fmtPer1000(tooltip.natMen)}
                        </span>
                      </div>
                    )}
                    {tooltip.natWomen != null && (
                      <div className="flex items-center justify-between gap-3">
                        <span className="flex items-center gap-1 text-gray-500 whitespace-nowrap">
                          ...
                          />
                          {femaleLabel}
                          {!hasRegional && userIsMale === false ? ' · you' : ''}
                        </span>
                        <span className="font-semibold text-gray-700">
                          {fmtPer1000(tooltip.natWomen)}
                        </span>
                      </div>
                    )}
                  </div>
              ...
              {(() => {
                ...
                if (
                  ...
                  const pct = Math.abs(
                    ((tooltip.regMen - tooltip.regWomen) /
                      Math.min(tooltip.regMen, tooltip.regWomen)) *
                      100
                  )
                  const higher =
                    tooltip.regMen > tooltip.regWomen ? maleLabel : femaleLabel
                  lines.push({
                    text:
                      pct < 5
                ...
                if (
                  ...
                  const pct = Math.abs(
                    ((tooltip.natMen - tooltip.natWomen) /
                      Math.min(tooltip.natMen, tooltip.natWomen)) *
                      100
                  )
                  const higher =
                    tooltip.natMen > tooltip.natWomen ? maleLabel : femaleLabel
                  lines.push({
                    text:
                      pct < 5
                ...
                return (
                  <div className="px-3 py-1.5 border-t border-gray-100 bg-gray-50 flex flex-col gap-0.5">
                    {lines.map((l, i) => (
                      <p key={i} className={`text-[10px] ${l.color}`}>
                        {l.text}
                      </p>
                    ))}
                  </div>
